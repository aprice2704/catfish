...
// Beeper makes a sine wave while awaiting commands
//
func beeper(device *alsa.Device, commands chan beepdo ) {
	mytime float
	defer device.Close()
...
func negotiate(device *alsa.Device) error {
	var err error
...
func beep_device(device *alsa.Device) error {
	...
	for {
		...
		for i := 0; i < buffer_size; i++ {
			v := math.Sin(t * 2 * math.Pi * 512) // A4
			v *= 0.05                            // make a little quieter
